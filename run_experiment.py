...
# Column names based on the readme.txt file
COLUMNS = [
...
def load_and_clean_data(filepath):
    ...
    Args:
        filepath (str): Path to the training or test data file (e.g., 'train_FD001.txt').
...
def calculate_train_rul(df):
    ...
    RUL is calculated as the maximum cycle count for an engine minus the current cycle count.
    """
    ...
    # Calculate RUL: (max cycle for this engine) - (current cycle)
...
def apply_piecewise_rul(df, cap=125):
    ...
    This is standard for C-MAPSS, assuming that once an engine is very healthy 
    (RUL > cap), its degradation signal is primarily noise.
...
def normalize_data(df_train, df_test):
    ...
    The same scaler is applied to the test data.
    """
...
def create_sequences(df, sequence_length, feature_cols, target_col=None):
    """
    Converts the time-series data into sequences for an RNN/LSTM model.
    """
    X, y, engine_indices = [], [], []
